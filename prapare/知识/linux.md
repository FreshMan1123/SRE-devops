...
怎么查看和调整进程的优先级
ps aux即可查看优先级，ni列为优先级
调整可以使用renice

CPU的cache缓存是什么
cpu的cache是位于cpu和内存之间的缓存区，主要用于存放最近cpu访问的数据以及指令

CPU的cache缓存，是怎么换入换出的，用的什么算法？
先入先出，优点是实现简单，缺点是没考虑命中频率
最近最少使用，优点是命中率高，缺点是硬件实现复杂
随即替换，优点是实现最简单，缺点是性能不确定

怎么看系统日志，你常看的日志有哪些
/var/log下存放日志，可使用journalctl命令查看日志
常用的日志有 messages系统信息日志，boot.log系统启动日志，auth.log认证日志，corn定时任务日志

linux是由内核和文件系统组成的，你知道哪些文件系统？
ext4，最常用的文件系统，性能好
xfs高性能文件系统，常用于数据库，支持在线扩容
NFS网络文件系统，用于网络文件共享
