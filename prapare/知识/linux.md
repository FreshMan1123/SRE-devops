...
inode是什么
inode是文件系统当中的索引节点，目录和文件都有inode，记录文件的创建时间，大小，权限等元数据

为什么硬链接不能链接到目录？这样设计的原因是什么？
因为硬链接链接到目录可能导致循环引用

网卡的作用是什么
网卡是连接网络的物理设备，唯一的mac地址使得外界可以找到服务器，并负责实际的数据传输

SSL跟TLS加密的区别是什么
SSL跟TLS都是加密数据的协议，用于加密算法的协商与交换密钥。TLS修复了SSL的漏洞，SSL已经废弃，目前主流是TLS

Cgroup通过不同的“子系统”（subsystem）来实现对不同资源的控制。你能列举几个你所知道的Cgroup子系统以及它们各自的作用吗？
cpu限制cpu的使用率
memory 限制内存的使用量
bikio 限制进程对块设备的IO速率
pids 限制一个控制组内可创建的进程数

写时复制是什么
当多个调用者访问同一个资源时，系统其实是给了它们一个指针指向该资源，共享的进行访问，只有当某个调用者需要进行写操作时，它才会复制对应的私有独立资源给那个调用者，其余调用者仍是使用该共享的资源

基于写时复制的原理，你认为它在fork()一个新进程时，带来的最大好处是什么？
提升创建进程的速度，当我们fork一个子进程时，先不需要完整的复制对应的内存，只需要给一个指向给内存的指针，当真正有写操作时，才复制一个独立的私有副本
...
熔断器的工作原理是什么？什么时候开启熔断，什么时候恢复？
关闭状态：正常转发请求，统计失败率
开启状态: 当失败率超过阈值时，一律不转发请求，返回空值或者错误
半开状态：允许少量请求测试服务是否恢复
...
用户态线程跟内核态线程的区别是什么
1. 用户态线程是应用程序自身创建的线程，对操作系统不可见
2. 内核态线程是我们实际理解中的线程，也就是操作系统真正调度的线程
